...
| Field | Value |
|-------|-------|
| Current version | `1.2.1` (stable — **patch**: closes the seven code-level items 1.2.0's P(-1) sweep carried forward. No API change, pin unchanged. One deliberate semantic refinement: kernel PCM state 8 now maps to `VANI_STATE_DISCONNECTED` instead of `VANI_STATE_UNKNOWN`) |
| Released | 2026-08-20 |
...
| Distribution profiles | full (`dist/vani.cyr`, 103,803 B / **109** symbols) and core (`dist/vani-core.cyr`, 43,830 B / **25** symbols) |
...
| Latest audit | [`docs/audit/2026-08-20-v1.2.1-audit.md`](../audit/2026-08-20-v1.2.1-audit.md) — closing addendum to the P(-1) sweep. Parent: [`2026-08-20-v1.2.0-audit.md`](../audit/2026-08-20-v1.2.0-audit.md) — 5 review lenses + adversarial verification of every non-INFO finding (50 of 55 re-rated) |
...
| Metric | Value |
|--------|-------|
| CPU test assertions | **778** (259 at 1.1.4, 775 at 1.2.0). Reference coverage **97%** — 106/109 fns, 8/8 files, up from 36/108 and 5/8. Every 1.2.0 repair ships a regression assertion, and the load-bearing ones were validated with negative controls (see CHANGELOG) |
...
| Artifact | Size | Notes |
|----------|------|-------|
| `dist/vani.cyr` (full profile) | 103,803 B (v1.2.1) | Full consumer-facing bundle: **109** public symbols. Grew from 1.1.4's 83,005 B — the 1.2.0 repairs plus the explanatory comment blocks each one carries. |
| `dist/vani-core.cyr` (core profile) | 43,830 B (v1.2.1) | Playback-only single-module bundle from `src/alsa.cyr`: **25** `audio_*` symbols (gained `audio_set_params_fmt`). |
| `build/vani_smoke` (DCE) | **515,320 B** (v1.2.1, cyrius 6.5.32) | x86_64 ELF link-check binary. Was 506,816 B at 1.1.4 — **+8,504 B**, entirely vani's own hardening (the pin bump is provably inert: byte-identical stdlib, byte-identical binary). |
